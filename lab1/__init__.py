...
class Lab1:
    ...
    def start(self):
        ...
        def run():
            ...
            if self.is_running:
                self.step()
                # Trigger a timer for next call
                self.root.after(self.delay_getter(), run)
    ...
    def make_env_frame(self):
        ...
        if (width != previous_width or height != previous_height) and self.grid:
            self.grid[self.marked_agent_pos[0]][self.marked_agent_pos[1]].configure(
        ...
        def make_button(x, y, container_frame):
            ...
            btn = Button(container_frame, text="", height=self.agent_btn_dims,
                         width=self.agent_btn_dims, padx=padx, pady=pady, image=self.blank_img)
            btn.pack(side="right")
    ...
    def draw_agent(self):
        self.grid[self.marked_agent_pos[0]][self.marked_agent_pos[1]
                                            ].configure(image=self.blank_img)
        self.grid[self.agent.location[0]][self.agent.location[1]
                                          ].configure(image=self.agent_img[self.agent.facing])
    ...
    def create_selection_menu(self, cb_on_select, *opts, always_trigger_onselect=False, no_destructure=False, pass_selection_to_callback=False):
        ...
        menu = OptionMenu(self.options_frame,
                          selection_active, *options_dict.keys())
    ...
    def create_sim(self):
        ...
        """
        venv = LIUVacuumEnvironment(
            *self.grid_dims_getter(), self.dirt_bias_getter(), self.wall_bias_getter(), self.seed_getter())
        ...
        if selected_agent == AGENT_MYVACUUMAGENT:
            ...
            agent = RandomVacuumAgent(
                *self.grid_dims_getter(), self.append_log)
        else:
...
def run(): return Lab1().start_main_loop()
